...
LOG_LEVEL_ENV_VAR = 'chipper_LOG'
BASE_LOGGER_NAME = 'chipper'
EXTENDED_DEBUG = 5
...
def formatter(time_utc=False):
    ...
    Returns
    -------
    A logging.Formatter object for chipper logging
    """
...
def setup_logger(level=logging.INFO, console_output=True, file_output=False,
    ...
    """
    Set up a new logging.Logger for chipper logging

    Calling this method will override any existing handlers, formatters etc.
    attached to the chipper logger. Typically, :func:`get_logger` should be
    used instead, which returns the existing chipper logger if it has already
    been set up, and can handle chipper submodule namespaces.
    ...
    Returns
    -------
    A logging.Logger object for chipper logging.
    :func:`get_logger`.
    """
    ...
    log.info('Logging started on chipper')
    if time_utc:
...
def get_logger(logger_name=BASE_LOGGER_NAME, log_level=None,
               **kwargs):
    """
    Returns (if extant) or creates a chipper logger

    If the chipper base logger has already been set up, this method will
    ...
    **kwargs : kwargs
        ...
        the chipper logger hasn't been set up yet (i.e. there have been no
    ...
    if BASE_LOGGER_NAME not in logging.Logger.manager.loggerDict.keys():
        ...
        warnings.warn('chipper logger already exists, ignoring keyword '
